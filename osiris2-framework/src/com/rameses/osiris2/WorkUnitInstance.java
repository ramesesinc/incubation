...
public class WorkUnitInstance implements Serializable {
    ...
    public void copyProperties( Map props ) {
        ...
        while(iter.hasNext()) {
            ...
            try {
                resolver.setProperty( getController(), me.getKey()+"", me.getValue() );
            } catch(Exception ex) {
    ...
    public void invoke(String method) {
        if( method != null && method.trim().length() > 0 ) {
            try {
                ...
                Object o = mr.invoke(getController(), method, null);
                if( o != null ) {
    ...
    public void setProperty(String name, Object value) {
        try {
            PropertyResolver.getInstance().setProperty( getController(), name, value );
        } catch(Exception ex){
            throw new IllegalStateException(ex.getMessage(), ex);
        }
    }
