...
public class CRUDController {
    ...
    public boolean isCanEdit() { 
        Object o = (entity != null? entity.get("state"): null); 
    ...
    public List getFormActions() {
        if (formActions == null) {
            ...
            //load create action
            Action a = createAction("create", "New", "images/toolbars/create.png", "ctrl N", 'n', "#{mode=='read' && allowCreate==true}", true);
            loadPermissionDefaults(a, "create");
            loadPermission(a, getCreatePermission());
            formActions.add(a);
            
            //load edit action
            a = createAction("edit", "Edit", "images/toolbars/edit.png", "ctrl E", 'e',  "#{mode=='read' && allowEdit==true && canEdit==true}", true);
            loadPermissionDefaults(a, "edit");
            loadPermission(a, getEditPermission());
            formActions.add(a);
            
            //load delete action
            a = createAction("delete", "Delete", "images/toolbars/trash.png", null, 'd', "#{mode=='read' && allowDelete==true && canDelete==true}", true);
            loadPermissionDefaults(a, "delete");
            loadPermission(a, getDeletePermission());
            formActions.add(a);
            
            //load approve action
            a = createAction("approve", "Approve", "images/toolbars/approve.png", null, 'v', "#{mode=='read' && allowApprove==true && canApprove==true}", true);
            loadPermissionDefaults(a, "approve");
    ...
    public Object save() {
        ...
        if (isShowConfirmOnSave()) {
            ...
            if (!MsgBox.confirm(msg)) return null;
        }
    ...
    private void loadPermission(Action a, Map perm) {
        ...
        String domain = getString(perm, "domain");
        if (domain != null && domain.length() > 0) {
            a.setDomain(domain); 
        }
        
        String role = getString(perm, "role");
        if (role != null && role.length() > 0) {
            a.setRole(role);
        }
        
        String permission = getString(perm, "permission"); 
    ...
    private void loadPermissionDefaults(Action a, String permission) {
        ...
        if ((domain == null || domain.length() == 0) && invoker != null) {
            domain = invoker.getDomain(); 
        }
        a.setDomain(domain); 
